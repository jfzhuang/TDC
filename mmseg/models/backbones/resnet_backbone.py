        ...
        if dilate_scale == 8:
            ...
            if multi_grid is None:
                ...
                for i, r in enumerate(multi_grid):
                    orig_resnet.layer4[i].apply(
                        partial(self._nostride_dilate, dilate=int(4 * r))
                    )
            ...
            if multi_grid is None:
                ...
                for i, r in enumerate(multi_grid):
                    orig_resnet.layer4[i].apply(
                        partial(self._nostride_dilate, dilate=int(2 * r))
                    )
...
def ResNetBackbone(
    backbone=None, pretrained=None, multi_grid=None, norm_type="batchnorm"
):
    arch = backbone
    if arch == "resnet34":
        ...
        orig_resnet = resnet34(pretrained=pretrained)
        arch_net = DilatedResnetBackbone(
            orig_resnet, dilate_scale=16, multi_grid=multi_grid
        )
        arch_net.num_features = 512
        ...
        orig_resnet = deepbase_resnet18(pretrained=pretrained, norm_type=norm_type)
        arch_net = DilatedResnetBackbone(
            orig_resnet, dilate_scale=8, multi_grid=multi_grid
        )

    elif arch == "deepbase_resnet50_dilated8":
        orig_resnet = deepbase_resnet50(pretrained=pretrained, norm_type=norm_type)
        arch_net = DilatedResnetBackbone(
            orig_resnet, dilate_scale=8, multi_grid=multi_grid
        )
