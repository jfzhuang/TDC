...
from mmseg.models.backbones.module_helper import ModuleHelper
...
class ResNet(nn.Module):
    def __init__(
        self, block, layers, num_classes=1000, deep_base=False, norm_type=None
    ):
        ...
        self.layer2 = self._make_layer(
            block, 128, layers[1], stride=2, norm_type=norm_type
        )
        self.layer3 = self._make_layer(
            block, 256, layers[2], stride=2, norm_type=norm_type
        )
        self.layer4 = self._make_layer(
            block, 512, layers[3], stride=2, norm_type=norm_type
        )
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m, ModuleHelper.BatchNorm2d(norm_type=norm_type, ret_cls=True)
            ):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, norm_type=None):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample, norm_type=norm_type)
        )
...
def deepbase_resnet18(
    num_classes=1000, pretrained=None, norm_type="batchnorm", **kwargs
):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on Places
